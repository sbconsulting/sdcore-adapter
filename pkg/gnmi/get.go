...
func (s *Server) Get(req *pb.GetRequest) (*pb.GetResponse, error) {
	...
	for i, path := range paths {
		...
		if !ok {
			/*
					smbaker: this makes no sense to me. The code is comparing the datatype,
						which is either "operational", "config", or "all" to the names of
						the elements in the path ("site", "device", ...). Unless the datatype
						is set to "all", it will fail.
